...
description: Field notes for developing DeepSeek Harness (dsh) plugins, focused on dual-face (Node host half + browser client half) plugins: dsh.bundle.patch one-command install, the dsh.client browser roster, /api exact routes, client HMR semantics, and a verify-against-installed-source methodology.
whenToUse: Load when building, modifying, or debugging DeepSeek Harness (dsh) plugins — especially web UI (browser-side) and dual-face plugins, bundle packaging/install, @deepseek-ai/dsh-* API usage — or when a plugin API claim needs verification against the locally installed dsh source.
---

# dsh Plugin Development Field Notes (dual-face perspective)

> **English edition.** The canonical version of this skill is [SKILL.md](SKILL.md) (Chinese); `references/` is currently Chinese-only. The fact tables and code blocks are language-neutral.
>
...
>
> Every template here ran for real on dsh `0.1.5-rc.2` and was re-verified against `0.1.6-alpha.2` (see `CHECKLIST.md`).
...
- `references/web-ui-plugins.md` — the full dual-face walkthrough with verification records (the main course; Chinese)
- `references/fact-sources.md` — the fact-source hierarchy and source-verification techniques (Chinese)
