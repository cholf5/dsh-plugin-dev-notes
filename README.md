...
[![dsh](https://img.shields.io/badge/verified%20on-dsh%200.1.5--rc.2-1f6feb)](#-与版本的关系)
...
Skill 是标准的 Agent Skills 格式（`SKILL.md` + YAML frontmatter），任何支持该格式的 agent 都能用。
...
所有事实在 `@deepseek-ai/dsh@0.1.5-rc.2` 上验证。dsh 处于快速演进期（0.x rc），**引用本笔记的 API 前请按 `references/fact-sources.md` 的手法对本地源码复核**——这条方法论比任何具体事实都保值。二手资料（包括本笔记）都可能过时，本机源码不会撒谎。
